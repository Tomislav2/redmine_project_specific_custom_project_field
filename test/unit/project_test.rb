...
class ProjectTest < ActiveSupport::TestCase
  ...
  def test_get_project_custom_field
    assert @project.project_specific_project_custom_fields.include?(@custom_field)
  end

  def test_destoy_project
    assert_difference ['ProjectCustomField.count', 'ProjectSpecificCustomFieldsProject.count'], -1 do
      @project.destroy()
    end
  end
