...
class Project < ActiveRecord::Base
  ...
  def available_custom_fields
    ...
    project_condition = " AND ( EXISTS (SELECT 1 FROM #{CustomField.table_name} ifa WHERE ifa.is_for_all = #{self.class.connection.quoted_true})" +
    ...
    print("\ntype = '#{self.class.name}CustomField'#{project_condition}\n")
