...
class ProjectSpecificFieldsControllerTest < ActionController::TestCase
  ...
  def test_create
    assert_difference ['ProjectCustomField.count'] do
          post :create, :id => @project.identifier, :p_spec_project_custom_field => { :name => 'new_custom_field', :field_format => 'float', :share_with_subprojects => true }
    end
        
    assert_equal 'new_custom_field', assigns(:custom_field).name
    assert_equal 'float', assigns(:custom_field).field_format
    assert_equal assigns(:custom_field), ProjectCustomField.last
  ...
  def test_create_no_name
    assert_difference ['ProjectCustomField.count'], 0 do
          post :create, :id => @project.identifier, :p_spec_project_custom_field => { :name => '', :field_format => 'float' }
    end
  ...
  def test_create_dont_share
    post :create, :id => @project.identifier, :p_spec_project_custom_field => { :name => 'new_custom_field', :field_format => 'float' }
    assert !assigns(:custom_field).share_with_subprojects?
  end
  
  def test_update
    post :update, :id => @custom_field.id, :p_spec_project_custom_field => { :name => 'rename_field', :field_format => 'string' }
  ...
  def test_destroy
    assert_difference ['ProjectCustomField.count'], -1 do
